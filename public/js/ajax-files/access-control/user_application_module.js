...
var UserApplicationModule = function () {
    ...
    return {
     // =========================================================================
        ...
        // =========================================================================
        init: function () {
            UserApplicationModule.handleCalls();
        },
        ...
        handleCalls: function() {
            ...
            $("#assignApplicationModuleGroup").click(function(e){
                UserApplicationModule.assignUserApplicationModuleToGroup();
            });
            
            $("#removeApplicationModuleGroup").click(function(e){
                UserApplicationModule.removeUserApplicationModuleFromGroup();
            });
...
//                    "serverSide": true,
                    'ajax': {
                    'url': getBaseURL + "/access/user-application-module/all"
                    },
                    pageLength: 10,
                    responsive: true,
                    ...
                    "aoColumns" : [
                                ...
                                {"render": function (data, type, full, meta) {
                                        return "<i class='"+full['moduleicon']['icon']+"'></i>";
                                }},
                             {"render": function (data, type, full, meta) {
                ...
                //---if edit ----- //
                $('.list-userapplicationmodule tbody').on('click', '.btn-edit', function (e) {
                    var $row = $(this).closest('tr');
                    var data = table.row($row).data();
                    var editurl = getBaseURL + "/access/user-application-module/" + data['id'] + "/edit";
                    
                    FlicksApp.handleRequestData(editurl,'edit');

                });
        },     
    assignUserApplicationModuleToGroup : function(){
            ...
            if (SelectedUnassign.length <= 0) {
                FlicksApp.render_msg("Please select at least one user application module to assign", '', '0');
                return;
            }
                
            $.ajax({
                url: getBaseURL + "/access/user-application-module/allot-group",
            ...
            success: function(data) { //alert(data);
                UserApplicationModule.loadUserApplicationModule();
                if(data.status === 1)
        ...
        // =========================================================================
        // Remove User Application Module from Access Group
        // =========================================================================
        removeUserApplicationModuleFromGroup : function(){
            ...
            $.ajax({
            url: getBaseURL + "/access/user-application-module/remove-group",
        ...
        // =========================================================================
        // Populate User Application Module
        // =========================================================================
        //------------ Load Group Privilege ---------- //
        loadUserApplicationModule : function(){ 
            
            FlicksApp.clear_diverror();//-------- clear error div ---- //
            var user_access_group_id = $("#user_access_group").val();
            $.ajax({
                url: getBaseURL + "/access/user-application-module/load-group",
