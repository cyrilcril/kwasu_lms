...
var FeeBillScheme = function () {
    ...
    return {
        ...
        handleCalls: function () {
            ...
            $("#assignFeeBillScheme").click(function (e) {
                FeeBillScheme.assignFeeBillScheme();
            });

            $("#removefee_bill_scheme").click(function (e) {
               FeeBillScheme.removeFeeBillScheme();
            });

        },

        handleFeeBillSchemeData: function () {

            $(".list-feebillscheme").dataTable().fnDestroy();
            var table = $('.list-feebillscheme').DataTable({
//                    "processing": true,
//                    "serverSide": true,
                'ajax': {
                    'url': getBaseURL + "/bursary/fee-bill-scheme/all"
                },
                pageLength: 10,
                responsive: true,
                dom: '<"html5buttons"B>lTfgitp',
                buttons: [
                    {extend: 'excel', title: 'FeeBillSchemeList'},
                    {extend: 'pdf', title: 'FeeBillSchemeList'},
                    {extend: 'print',
                ...
                "aoColumns": [
                    {mData: "fee_bill_scheme"},
                     {mData: "fee_bill_scheme_short_name"},
                      {"render": function (data, type, full, meta) {
                            return full['feetype']['fee_type'];
                        }},
                    {mData: "fee_bill_scheme_description"},
            ...
            //---if edit ----- //
            $('.list-feebillscheme tbody').on('click', '.btn-edit', function (e) {
                var $row = $(this).closest('tr');
                var data = table.row($row).data();
                var editurl = getBaseURL + "/bursary/fee-bill-scheme/" + data['id'] + "/edit";
        ...
        assignFeeBillScheme: function () {
            ...
            $.ajax({
                url: getBaseURL + "/bursary/fee-bill-scheme/allot-group",
                ...
                success: function (data) {
                    FeeBillScheme.loadFeeBillScheme();
                    if (data.status === 1)
        ...
        // =========================================================================
        // Remove Fee Bill Scheme
        // =========================================================================
        removeFeeBillScheme: function () {
            ...
            $.ajax({
                url: getBaseURL + "/bursary/fee-bill-scheme/remove-group",
        ...
        // =========================================================================
        // Populate Fee Bill Schemes
        // =========================================================================
        //------------ Load Olevel Sujects ---------- //
        loadFeeBillScheme: function () {
            FlicksApp.clear_diverror();
            var academic_course_option_standard_id = $("#academic_course_option_standard").val();
            $.ajax({
                url: getBaseURL + "/bursary/fee-bill-scheme/load-group",
